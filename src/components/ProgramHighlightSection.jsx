            ...
            {bullets?.length ? (
              ...
              >
                {bullets.map((bullet) => (
                  <motion.li
                    key={bullet}
                    className="ht-motion-smooth group flex items-start gap-3 rounded-2xl border border-ht-silver bg-white/90 px-4 py-3 shadow-[0_16px_34px_-30px_rgba(5,42,74,0.45)] backdrop-blur-sm"
              ...
              <motion.div className="mt-5 space-y-3" {...getStaggerContainer(reduceMotion, { staggerChildren: 0.07, amount: 0.2 })}>
                {rightItems.map((item, index) => (
                  <motion.div
                    ...
                    className="ht-motion-smooth group rounded-2xl border border-ht-silver bg-white px-4 py-4 shadow-[0_14px_30px_-28px_rgba(5,42,74,0.4)]"
