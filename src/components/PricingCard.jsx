...
function PricingCard({
  ...
  return (
    <motion.article
      className={`ht-motion-smooth flex h-full flex-col rounded-3xl border p-6 shadow-sm hover:-translate-y-1 hover:shadow-xl md:p-7 ${
        featured
    ...
    >
      <h3 className="text-ht-navy">{name}</h3>

      {price ? (
        <p className="mt-2 text-[clamp(1.75rem,2.35vw,2.5rem)] font-extrabold leading-none text-ht-navy">
          {price}
      ...
      {priceLines.length ? (
        <ul className="mt-5 space-y-2 ht-body text-ht-gray">
          {priceLines.map((line) => (
      ...
      {features.length ? (
        <ul className="mt-6 space-y-2 ht-body text-ht-gray">
